  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <div
          className="rounded-2xl p-5 text-left text-sm mb-5"
          style={{ background: 'var(--bg-raised)', border: '1px solid var(--border)' }}
        >
          <p className="font-bold mb-2" style={{ color: 'var(--text-heading)' }}>📋 Ringkasan Booking</p>
          <p style={{ color: 'var(--text)' }}>Layanan: <strong>{form.service}</strong></p>
          <p style={{ color: 'var(--text)' }}>Dokter: <strong>{selectedDoctor?.name ?? '—'}</strong></p>
          <p style={{ color: 'var(--text)' }}>Tanggal: <strong>{form.date}</strong></p>
          <p style={{ color: 'var(--text)' }}>Waktu: <strong>{form.time}</strong></p>
        </div>
        <button
      ...
      <div className="flex items-center justify-center gap-3 mb-8">
        {[1, 2].map((s) => (
          <div key={s} className="flex items-center gap-2">
            ...
            <span className="text-sm" style={{ color: step >= s ? 'var(--text-heading)' : 'var(--text)' }}>
            ...
            <div>
              ...
              <div className="space-y-2">
                {doctors.map((doc) => (
                  ...
                  >
                    <p className="font-semibold text-sm" style={{ color: 'var(--text-heading)' }}>{doc.name}</p>
